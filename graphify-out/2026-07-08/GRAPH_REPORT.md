...
## Corpus Check
- Corpus is ~7,130 words - fits in a single context window. You may not need a graph.

## Summary
- 105 nodes · 141 edges · 14 communities (10 shown, 4 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 17 edges (avg confidence: 0.76)
- Token cost: 0 input · 41,754 output
...
1. `MSLGUI` - 12 edges
2. `find_msl()` - 9 edges
3. `DigiKeyAPIError` - 7 edges
4. `cache_msl()` - 7 edges
5. `get_cached_msl()` - 7 edges
6. `get_access_token()` - 5 edges
7. `search_part()` - 5 edges
8. `search_part_retry()` - 5 edges
9. `get_connection()` - 5 edges
10. `search_part_retry()` - 5 edges

## Surprising Connections (you probably didn't know these)
- `bool` --uses--> `DigiKeyAPIError`  [INFERRED]
  msl_lookup/msl_finder.py → msl_lookup/digikey_api.py
- `MSL-LookUp Root README` --references--> `MSL Lookup Tool README`  [INFERRED]
  README.md → msl_lookup/README.md
...
Cohesion: 0.50
Nodes (5): DigiKey Data Source, Maker Part Number (MPN), Moisture Sensitivity Level (MSL), MSL Lookup Tool README, MSL-LookUp Root README
...
## Knowledge Gaps
- **15 isolated node(s):** `allow`, `int`, `float`, `int`, `float` (+10 more)
...
- **Are the 4 inferred relationships involving `get_cached_msl()` (e.g. with `run_lookup()` and `search_single()`) actually correct?**
  _`get_cached_msl()` has 4 INFERRED edges - model-reasoned connections that need verification._